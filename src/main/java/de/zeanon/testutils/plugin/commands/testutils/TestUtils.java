...
public class TestUtils extends SWCommand {
		...
		return new TypeMapper<MappedFile>() {
			...
			public java.util.List<String> tabCompletes(final @NotNull CommandSender commandSender, final @NotNull String[] previousArguments, final @NotNull String arg) {
				if (commandSender instanceof Player) {
					...
					try {
						final @NotNull Path filePath = de.zeanon.testutils.TestUtils.getInstance().getDataFolder().toPath().resolve("TestBlocks/" + p.getUniqueId() + path).toRealPath();
						final @NotNull Path basePath = de.zeanon.testutils.TestUtils.getInstance().getDataFolder().toPath().resolve("TestBlocks/" + p.getUniqueId()).toRealPath();
	...
	private @NotNull TypeMapper<AreaName> mapArea() {
		return new TypeMapper<AreaName>() {
			...
			public java.util.List<String> tabCompletes(final @NotNull CommandSender commandSender, final @NotNull String[] previousArguments, final @NotNull String arg) {
				try {
					return BaseFileUtils.listFolders(de.zeanon.testutils.TestUtils.getInstance().getDataFolder().toPath().resolve("TestAreas").toRealPath().toFile()).stream().map(File::getName).collect(Collectors.toList());
				} catch (IOException e) {
					return null;
				}
			}
