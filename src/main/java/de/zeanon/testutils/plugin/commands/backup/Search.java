				...
				if (!regionFolder.exists()) {
					...
					if (modifiers.getFileName() == null) {
						...
						if (modifiers.getBackUpMode() == BackUpMode.NONE) {
							files.put("manual", BaseFileUtils.searchFolders(new File(regionFolder, "manual/" + p.getUniqueId()), modifiers.getFileName())
															 .stream()
															 .sorted((f1, f2) -> Long.compare(f2.lastModified(), f1.lastModified()))
															 .collect(Collectors.toList()));
							files.put("hourly", BaseFileUtils.searchFolders(new File(regionFolder, "automatic/hourly"), modifiers.getFileName()).stream()
															 .sorted((f1, f2) -> Long.compare(f2.lastModified(), f1.lastModified()))
															 .collect(Collectors.toList()));
							files.put("daily", BaseFileUtils.searchFolders(new File(regionFolder, "automatic/startup"), modifiers.getFileName()).stream()
															.sorted((f1, f2) -> Long.compare(f2.lastModified(), f1.lastModified()))
															.collect(Collectors.toList()));
							files.put("startup", BaseFileUtils.searchFolders(new File(regionFolder, "automatic/startup"), modifiers.getFileName()).stream()
															  .sorted((f1, f2) -> Long.compare(f2.lastModified(), f1.lastModified()))
															  .collect(Collectors.toList()));
						} else if (modifiers.getBackUpMode() == BackUpMode.MANUAL) {
							files.put(modifiers.getBackUpMode().toString(), BaseFileUtils.searchFolders(new File(regionFolder, modifiers.getBackUpMode().getPath() + "/" + p.getUniqueId()), modifiers.getFileName()).stream()
																						 .sorted((f1, f2) -> Long.compare(f2.lastModified(), f1.lastModified()))
																						 .collect(Collectors.toList()));
						} else {
							files.put(modifiers.getBackUpMode().toString(), BaseFileUtils.searchFolders(new File(regionFolder, modifiers.getBackUpMode().getPath()), modifiers.getFileName()).stream()
																						 .sorted((f1, f2) -> Long.compare(f2.lastModified(), f1.lastModified()))
																						 .collect(Collectors.toList()));
						}
