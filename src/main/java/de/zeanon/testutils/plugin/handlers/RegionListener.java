...
public class RegionListener implements Listener {
	...
	public void onExplosionPrime(final @NotNull ExplosionPrimeEvent event) {
		for (final @NotNull Region region : RegionManager.getApplicableRegions(event.getEntity().getLocation())) {
			if (region.stoplag()) {
				event.setCancelled(true);
				return;
			}
	...
	public void onBlockPlace(final @NotNull BlockMultiPlaceEvent event) {
		...
		for (final @NotNull BlockState tempState : event.getReplacedBlockStates()) {
			for (final @NotNull Region region : RegionManager.getApplicableRegions(tempState.getLocation())) {
				region.setHasChanged(true);
			}
		}
	}
	...
	public void onInteract(final @NotNull PlayerInteractEvent event) {
		if (event.getClickedBlock() != null) {
			for (final @NotNull Region region : RegionManager.getRegions()) {
	...
	public void onInventoryMoveItem(final @NotNull InventoryMoveItemEvent event) {
		...
		if (destination != null) {
			for (final @NotNull Region region : RegionManager.getApplicableRegions(destination)) {
				if (region.stoplag()) {
					event.setCancelled(true);
					return;
				}
		...
		if (initiation != null) {
			for (final @NotNull Region region : RegionManager.getApplicableRegions(initiation)) {
				if (region.stoplag()) {
					event.setCancelled(true);
					return;
				}
