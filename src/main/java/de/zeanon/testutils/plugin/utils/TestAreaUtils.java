...
public class TestAreaUtils {
	...
	public @Nullable DefinedRegion getRegion(final @NotNull Player p) {
		List<DefinedRegion> regions = de.zeanon.testutils.plugin.utils.region.RegionManager.getApplicableRegions(p.getLocation());
		if (regions.isEmpty()) {
			return null;
		} else {
	...
	public @Nullable DefinedRegion getRegion(final @NotNull Player p, final @NotNull RegionSide regionSide) {
		switch (regionSide) {
			case NORTH:
	...
	public @Nullable DefinedRegion getOppositeRegion(final @NotNull Player p) {
		final Optional<DefinedRegion> temp = de.zeanon.testutils.plugin.utils.region.RegionManager.getApplicableRegions(p.getLocation()).stream().findFirst();
	...
	public @Nullable DefinedRegion getOppositeRegion(final @NotNull Player p, final @NotNull RegionSide regionSide) {
		switch (regionSide) {
			case NORTH:
				return TestAreaUtils.getSouthRegion(p);
			case SOUTH:
	...
	public @Nullable DefinedRegion getNorthRegion(final @NotNull org.bukkit.World world, final @NotNull String name) {
		return RegionManager.getRegion(name + "_north", world);
	}

	public @Nullable DefinedRegion getSouthRegion(final @NotNull org.bukkit.World world, final @NotNull String name) {
